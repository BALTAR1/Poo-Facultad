...
class Vehiculo:
    ...
    def calcular_importe_venta_usado(self):
        ...
        if self.anio:
            precio_venta -= (
                2023 - self.anio
            ) * 1000  # Restar 1000 por cada año de antigüedad
        if self.kilometraje:
            precio_venta -= (
                self.kilometraje * 0.1
            )  # Restar 0.1 por cada kilómetro recorrido
        return precio_venta
        ...
        if self.__usado:
            return (
                ...
                * (1 - 0.01 * (datetime.datetime.now().year - self.__año))
                - self.__precio_base * 0.02 * (self.__kilometraje - 100000)
                if self.__kilometraje > 100000
                else self.__precio_base
                * (1 - 0.01 * (datetime.datetime.now().year - self.__año))
            )
        else:
            return (
                self.__precio_base
                + self.__precio_base * 0.10
                + self.__precio_base * 0.02
                if self.__version == "full"
                else self.__precio_base + self.__precio_base * 0.10
            )
