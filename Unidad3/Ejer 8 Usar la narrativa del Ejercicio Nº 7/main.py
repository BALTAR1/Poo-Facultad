...
from claseDocenteInvertigador import DocenteInvestigador
...
def menu_tesorero(tesorero: ITesorero, lista_agentes: Lista):
    while True:
        ...
        if opcion == 1:
            ...
            if gasto_sueldo:
                print(
                    f"Gasto de sueldo para el empleado con CUIL {cuil}: {gasto_sueldo}"
                )
            else:
...
def crear_agente():
    ...
    if tipo_agente == 1:
        ...
        docente = Docente(
            cuil, apellido, nombre, sueldo_basico, antiguedad, carrera, cargo, catedra
        )
        return docente
    elif tipo_agente == 2:
        ...
        area_investigacion = input("Ingrese el área de investigación: ")
        tipo_investigacion = input("Ingrese el tipo de investigación: ")
        categoria_incentivos = input(
            "Ingrese la categoría en el programa de incentivos de investigación: "
        )
        importe_extra = float(
            input("Ingrese el importe extra por docencia e investigación: ")
        )
        ...
        personal_apoyo = PersonalApoyo(
            cuil, apellido, nombre, sueldo_basico, antiguedad, categoria
        )
...
def menu(lista_agentes: Lista):
    while True:
        ...
        print(
            "6. Recorrer la colección y generar listado con nombre, apellido, tipo de Agente y sueldo"
        )
        ...
        if opcion == 1:
            agente = crear_agente()
            posicion = int(
                input("Ingrese la posición en la que desea insertar el agente: ")
            )
            lista_agentes.agregar_agente_a_posicion(posicion, agente)
            print(
                f"Agente {agente.get_nombre()} {agente.get_apellido()} insertado en la posición {posicion}."
            )
        elif opcion == 2:
            agente = crear_agente()
            lista_agentes.agregar_agente(agente)
            print(
                f"Agente {agente.get_nombre()} {agente.get_apellido()} agregado a la colección."
            )
        elif opcion == 3:
            ...
            docentes_investigadores = (
                lista_agentes.generar_listado_docentes_investigadores_por_area(
                    area_investigacion
                )
            )
            for docente_investigador in docentes_investigadores:
                print(
                    docente_investigador.get_apellido(),
                    docente_investigador.get_nombre(),
                    docente_investigador.get_cargo(),
                    docente_investigador.get_catedra(),
                )
        elif opcion == 5:
            categoria_investigacion = input("Ingrese la categoría de investigación: ")
            agentes_por_categoria = lista_agentes.generar_listado_agentes_por_categoria(
            ...
            for nombre, apellido, cargo, cátedra, sueldo in listado:
                print(
                    f"{nombre} {apellido} - Cargo: {cargo} - Cátedra: {cátedra} - Sueldo: {sueldo}"
                )
        elif opcion == 7:
