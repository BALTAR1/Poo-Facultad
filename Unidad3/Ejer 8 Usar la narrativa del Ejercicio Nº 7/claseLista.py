...
class Lista:
    ...
    def generar_listado_docentes_investigadores_por_area(
        self, area_investigacion: str
    ) -> List[DocenteInvestigador]:
        listado = []
        i = 0
        while i < len(self.__agentes):
            agente = self.__agentes[i]
            if (
                isinstance(agente, DocenteInvestigador)
                and agente.__área_investigación == area_investigacion
            ):
    ...
    def generar_listado_agentes_por_categoria(self, categoria_investigacion):
        ...
        while i < len(self.__agentes):
            ...
            if (
                isinstance(agente, DocenteInvestigador)
                and agente.get_categoria_programa() == categoria_investigacion
            ):
                listado.append(agente)
            i += 1
        return listado
    ...
    def recorrer_lista_e_generar_listado(self):
        ...
        while i < len(self.__agentes):
            ...
            listado.append(
                (agente.get_nombre(), agente.get_apellido(), cargo, catedra, sueldo)
            )
            i += 1
        return listado
